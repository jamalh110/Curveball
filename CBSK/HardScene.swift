...
class HardScene: SKScene {
    ...
    override func update(_ currentTime: TimeInterval) {
            ...
            if(delta<2||playerLives==0){
                ...
                if(oppLives==0){
                    ...
                    if(Int(level)==11){
                        ...
                        variables.scoreHard = score
                        let transition = SKTransition.fade(withDuration: 0.5)
                        
                        let nextScene = WinSceneHard(size: scene!.size)
                        nextScene.scaleMode = .aspectFill
        ...
        if(ball.zSpeed>0 && ball.vec1.z>12.5){
        oppPaddle.moveTo((level*3.2-1)*delta, loc: ball.midLoc())
        }
        else{
            oppPaddle.moveTo((level*3.2-1)*delta, loc: Vector(x: 0, y: 0, z: 0))
        }
        ...
        if(ball.vec1.z<=8){
            ...
            if(ball.vec1.x>playerPaddle.vec3.x||ball.vec3.x<playerPaddle.vec1.x||ball.vec1.y>playerPaddle.vec3.y||ball.vec3.y<playerPaddle.vec1.y){
                ...
                if(playerLives==0){
                    ...
                    let nextScene = GameOverHard(size: scene!.size)
                    nextScene.scaleMode = .aspectFill
